...
```
➜  egg-oop-parser-solution git:(master) ✗ ln -s ~/campus-virtual/shared/egg/oop-evm-releases/evm-2122-macos ./evm
``` 

and try with some example:

```js
➜  egg-oop-parser-solution git:(master) ✗ cat examples/array.egg 
do(
  def(x, arr(arr(1,4),5,7)),
  print(element(x,0)), # [1,4]
  print(element(x,1))  # 5
)
➜  egg-oop-parser-solution git:(master) ✗ ./evm examples/array.json 
[1,4]
5
```
